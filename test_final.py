#!/usr/bin/env python3
"""最终验证：模拟真实 generateScheme 流程，测试模式2在不同主色下的输出"""
import math
...
# 模拟完整 generateScheme (auto mode)
def simulate(primary):
    ...
    return {
        'primary': primary,
        'box_back': back,
        'hilited_text': hilited_text,
        'hilited_back': hilited_back,
        'hilited_label': hilited_label,
        'hilited_comment': hilited_comment,
        'contrast_t_b': contrast_ratio(hilited_text, hilited_back),
        'contrast_l_b': contrast_ratio(hilited_label, hilited_back),
        'contrast_c_b': contrast_ratio(hilited_comment, hilited_back),
    }

print("="*80)
print("模式2 最终输出表 (主色用于'选定候选项文字')")
print("="*80)
print(f"{'主色':<10} {'box背景':<10} {'hilited_back':<12} {'文字':<10} {'序号':<10} {'提示':<10} {'文字对比度':<10}")
print("-"*80)

for color in ['CD276E', 'E91E63', '2196F3', '3F51B5', '0D47A1', '4CAF50', 'FFEB3B', '795548']:
    r = simulate(color)
    print(f"#{r['primary']:<9} #{r['box_back']:<9} #{r['hilited_back']:<11} #{r['hilited_text']:<9} #{r['hilited_label']:<9} #{r['hilited_comment']:<9} {r['contrast_t_b']:.2f}:1")